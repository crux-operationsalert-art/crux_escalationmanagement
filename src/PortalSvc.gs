/**
 * PortalSvc.gs — read-only, tokenised client-facing portal (server side).
 *
 * NAMED PortalSvc, NOT Portal, DELIBERATELY. Apps Script identifies files by
 * their base name with the extension stripped, so `Portal.gs` and `Portal.html`
 * collide and the editor refuses the second one. The HTML file must keep the
 * name `Portal` because Code.gs:51 resolves it via
 * HtmlService.createTemplateFromFile('Portal'). Renaming this .gs file is free:
 * Apps Script has no imports and every .gs shares one global scope, so no code
 * anywhere refers to this filename. Do not rename it back.
 *
